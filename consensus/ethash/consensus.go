...
func CalcDifficulty(config *params.ChainConfig, time, parentTime uint64, parentDifficulty *big.Int, parentNumber uint64, parentUncleHash common.Hash) *big.Int {
	...
	case config.IsEthPoWFork(next):
		if config.EthPoWForkBlock != nil && (big.NewInt(0).Add(config.EthPoWForkBlock, big.NewInt(2048))).Uint64() == next {
			return params.ETHWStartDifficulty //Reset difficulty
		}

		if config.EthPoWForkBlock != nil && config.EthPoWForkBlock.Uint64() == next {
			return big.NewInt(1) //Reset
		}
		return calcDifficultyEthPoW(time, parentTime, parentDifficulty, parentUncleHash)
...
func calcDifficultyEthPoW(time uint64, parentTime uint64, parentDifficulty *big.Int, parentUncleHash common.Hash) *big.Int {
